...
export default function AccountBookPage() {
  ...
  if (loadError || !store) {
    return (
      <StLoadingPage>
        <StLoadingCard>{loadError || "가계부 데이터를 불러오지 못했습니다."}</StLoadingCard>
      </StLoadingPage>
    );
  }
    ...
    <>
      ...
      {isSettingsOpen ? (
        <AccountBookLockGate
          ...
          description={`${settingsManagerUser?.name || "관리자"}만 설정 화면에 접근할 수 있습니다.`}
          backToHome={false}
        ...
        >
          <WorkspaceSettingsModal
            ...
            users={store.users}
            sharedWorkspaces={store.workspaces.filter((workspace) => workspace.type === "shared")}
