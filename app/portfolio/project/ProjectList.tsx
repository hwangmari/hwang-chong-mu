...
export default function ProjectList() {
  ...
  return (
    <StProjectList>
      ...
      <ProjectCard
        anchorId="toy-travel"
        title="여행 플랜 (날짜별 동선·이동 시간)"
        period="2026.09 - 진행 중 (1인 개발)"
        linkUrl="/travel"
        description={
          <>
            출발·도착 날짜를 정하면 <b>DAY 탭이 하루씩</b> 만들어지고, 하루마다
            들를 곳을 담아 순서를 바꾸는 여행 일정표입니다.
            <br />
            순서를 바꾸면 지도에 <b>번호 핀과 선</b>이 다시 그려지고, 장소와
            장소 사이마다 <b>도보·대중교통·차량 이동 시간</b>이 붙습니다.
            <br />
            로그인 없이 <b>주소(링크) 하나</b>로 같이 고칠 수 있고, 일정을 글로
            복사해 메신저에 그대로 붙여 넣을 수 있습니다.
          </>
        }
        details={{
          problem: (
            <>
              여행 일정은 메모장과 지도, 검색창을 오가며 짜게 됩니다.
              <StDetailList>
                <li>
                  갈 곳을 적어 둬도 <b>어디서 어디까지 얼마나 걸리는지</b>는
                  매번 따로 찾아봐야 함
                </li>
                <li>순서를 바꾸면 이동 시간이 전부 달라지는데 손으로 다시 계산</li>
                <li>여럿이 같이 고치려면 파일이나 화면을 주고받아야 함</li>
              </StDetailList>
            </>
          ),
          solution: (
            <>
              하루 단위로 장소를 쌓고, <b>순서를 바꾸면 지도와 이동 시간이 같이</b>{" "}
              따라오게 했습니다.
              <StDetailList>
                <li>
                  기간을 정하면 DAY 탭이 자동 생성 — 기간을 늘려도 이미 적어 둔
                  날의 내용은 그대로 유지
                </li>
                <li>
                  순서 바꾸기는 <b>끌어 옮기기와 ▲▼ 버튼</b> 두 가지라 휴대폰에서도
                  가능
                </li>
                <li>
                  그날 묵는 숙소는 맨 위에 고정하고, 동선(이동 시간 계산)에서 뺄지
                  켜고 끌 수 있음
                </li>
                <li>
                  장소마다 메모, 구간마다 <b>도보·대중교통·차량</b> 이동 시간을
                  골라서 조회
                </li>
                <li>
          ...
          tech: (
            <>
              <StDetailList>
                <li>
                  Next.js App Router + styled-components, 하루치 일정은{" "}
                  <b>jsonb 한 칸</b>에 담고 <b>그 날짜 칸만 바꾸는 함수(RPC)</b>로
                  저장해 같은 시각에 다른 날을 고쳐도 서로 지워지지 않게 함
                </li>
                <li>
                  구글 <b>Places API (New)</b> 장소 검색과 <b>Routes API</b>{" "}
                  길찾기는 <b>서버 주소를 거쳐</b>서만 부르고, 열쇠는 서버에 두고
                  호출 횟수를 제한
                </li>
                <li>
                  지도는 <b>Google Maps JavaScript API</b>를 브라우저에 한 번만
                  싣고, 열쇠가 없으면 지도 대신 요약 카드로 내려가게 설계
                </li>
                <li>
