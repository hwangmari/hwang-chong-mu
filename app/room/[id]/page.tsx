...
import { supabase } from "@/lib/supabase"; // Supabase 설정 파일 경로 확인
import {
  format,
  addDays,
  isSameDay,
...
export default function RoomDetail() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // DB의 날짜 문자열(YYYY-MM-DD)을 Date 객체로 변환하여 상태 저장
  ...
  // --- [4] 날짜 계산 로직 (작성해주신 로직 그대로 유지) ---
  // start_date가 없으면 오늘을 기준으로 하되, 로딩 중엔 빈 배열 처리
  ...
  const handleSubmitVote = () => {
    ...
    const saveAction = async () => {
      try {
        // Date 객체 배열을 문자열(YYYY-MM-DD) 배열로 변환
        const dateStrings = currentUnavailable.map((d) =>
          format(d, "yyyy-MM-dd")
        );

        // 이미 같은 이름이 있는지 확인 (있으면 덮어쓰기 로직 구현 가능하지만, 여기선 단순 추가 or 알림)
        // 편의상 기존에 같은 이름이 있으면 삭제하고 다시 넣는 방식이 깔끔합니다.
        // (실제 서비스에선 id로 구분하지만, 토이프로젝트라 이름으로 구분)

        // 1. 기존 데이터 삭제 (이름 중복 방지)
        await supabase
          .from("participants")
          .delete()
          .eq("room_id", roomId)
  ...
  const handleReset = () => {
    // DB 삭제까지 할지, 뷰만 리셋할지 결정. 여기선 뷰만 리셋.
        ...
        >
          ...
          {step === "VOTING" && currentName.length > 0 && (
            <div className="mt-2 text-center text-xs text-gray-400 flex justify-center gap-3">
              ...
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-gray-300"></span> 다른
                사람 불가
              </span>
            </div>
          )}
