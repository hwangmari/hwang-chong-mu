...
type Props = { initialEvent: TennisEvent };

// 교류전(개인 승점) 화면. 이벤트 불러오기는 [id]/page.tsx가 맡고, 여기서는 그 뒤의 모든 상태를 다룬다
export default function ExchangeView({ initialEvent }: Props) {
    ...
    <StPage>
      ...
      {editing ? (
        <StCard>
          ...
          <StCardHint>
            선수를 바꾸거나, 경기를 끌어다 놓아(또는 ▲▼) 순서를 바꿀 수 있어요. 경고가 있으면 아래에
            추천 순서 변경이 떠요. 저장하면 모두에게 반영돼요. 이미 시작한 경기는 🔒로 잠겨요.
          </StCardHint>
          <BracketEditor
            players={event.players}
            ...
            courts={event.courts}
            rules={event.rules}
            locked={new Set(Object.keys(scores).map(Number))}
