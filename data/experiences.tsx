...
const HANWHA_EXPERIENCE: Experience = {
  id: "hanwha",
  company: "한화생명",
  ...
  summary: [
    ...
    "HSP 상담 플랫폼 레거시 마이그레이션 (Legacy → Next.js)",
    "D2F, 전자명함 등 온/오프라인 연계(O2O) 비즈니스 서비스 확장",
    "보장분석 서비스 고도화 및 보장분석 전용 HSP 상담 플로우 구축",
  ],
  workSummary: [
    "견고한 UI 마크업 역량을 기반으로 프론트엔드 개발자로 성공적으로 안착하여 전문성을 확보했습니다. \n경험을 바탕으로 모호한 기획을 명확한 개발 스펙으로 구체화하고, 효율적인 리소스 관리와 협업 프로세스를 주도하여 서비스 운영 안정성을 높입니다.",
  ],
  projects: [
    {
      title: "보장분석 서비스 운영 전담",
      period: "2026.01 ~ 상시 운영",
      description:
        "보장분석 서비스의 프론트엔드 개발을 전담하며, 운영 이슈 대응부터 기능 고도화까지 안정적인 서비스 품질을 유지",
      tasks: [
        "운영 이슈의 우선순위 판단 및 신속한 대응을 통한 서비스 안정성 확보",
        "요구사항 분석 및 프론트엔드 개발/배포 전 과정 수행",
        "보장분석 결과 리포트 고도화 및 GA 이벤트 흐름 정비로 단계별 전환 측정",
        "보장분석 결과에서 이어지는 전용 HSP 상담 플로우 구축 (분석 맥락을 상담 요청에 전달)",
      ],
      techStack: [
        "Next.js",
        "React",
    ...
    },
    {
      ...
      period: "2025.04 ~ 상시 운영",
      description:
        "플랫폼 오픈 이후, 온/오프라인 연계(O2O) 및 고객 행동 기반의 타겟팅 기능을 지속적으로 확장하여 비즈니스 가치(CVR)를 창출하고 시스템 안정성을 확보",
      tasks: [
        "고객 행동 데이터(체류시간, 화면 전환 등)를 실시간 분석하여 적시에 상담을 유도하는 '이탈 감지 로직' 구현",
        "상품 탐색, 사고보험금 등 주요 서비스 접점 내 상담 프로세스를 내재화하여 고객 경험 단절 해소",
        "App(In-webview)과 Mobile Web 간 인터랙션 차이를 극복하는 확장성 있는 구조 설계 (상품찾기 메인 탭 연동)",
        "이벤트 및 만족도 조사 프로세스 내 본인인증/보안 로직 고도화 및 잔존 레거시 기능의 지속적 마이그레이션",
      ],
      techStack: [
        "Next.js",
        "React",
    ...
    },
    {
      ...
      description:
        "노후화된 JSP 기반의 레거시 시스템을 Next.js(CSR) 환경으로 전면 재구축하여 사용자 경험(SPA)을 개선하고, \n 컴포넌트 기반 아키텍처 도입으로 유지보수 효율성 확보",
      tasks: [
        "JSP 기반의 강결합 구조를 해소하고, Next.js 및 컴포넌트 단위의 모듈화 설계를 통해 재사용성 증대",
        "React-Query의 캐싱 및 조건부 페칭(지역 선택 시 FP 호출) 전략을 도입하여 초기 렌더링 속도 획기적 개선",
        "TypeScript 도입 및 비즈니스 로직(Custom Hooks)과 UI의 철저한 분리로 런타임 에러 사전 방지",
        "외부 유입(광고 배너 등) 시 발생하던 UTM 파라미터 유실 문제를 근본적으로 해결하여 마케팅 데이터 정합성 확보",
        "구전으로 전해지던 레거시 히스토리와 복잡한 로직을 Figma로 시각화/문서화하여 인적 의존도 제거 및 운영 리스크 최소화",
      ],
      techStack: [
        "Next.js",
        "React",
    ...
    },
    {
      ...
      tasks: [
        ...
        "백오피스 상품 관리 기능 고도화를 통한 현업 운영 리소스 절감",
      ],
      techStack: [
        "Next.js",
        "React",
...
const KAKAO_EXPERIENCE: Experience = {
  id: "kakao-ent",
  company: "카카오 엔터프라이즈",
  role: "워크개발 / 마크업개발파트",
  ...
  workSummary: [
    ...
    "4가지 OS 환경에 대응하는 크로스 플랫폼 웹뷰 개발을 통해, 네이티브 앱과 이질감 없는 고품질의 웹 경험을 구현하는 데 주력했습니다.",
  ],
  projects: [
    {
      title: "카카오워크 투표 (Vote)",
...
const MUSINSA_EXPERIENCE: Experience = {
  id: "musinsa",
  company: "29CM(무신사)",
  ...
  workSummary: [
    ...
    "PC/Mobile/Webview 등 다양한 디바이스 환경에 최적화된 고품질 반응형 웹 시스템 구축을 주도하여 플랫폼 접근성을 획기적으로 개선했습니다. \n",
    "단순한 화면 구현을 넘어, 유지보수와 확장성을 고려한 UI 컴포넌트 구조를 설계하여 코드의 재사용성을 높이고 운영 효율을 극대화했습니다. \n",
    "서비스 전면 개편부터 고도화된 인터랙션이 필요한 브랜드 캠페인(PT)까지 전담하며, 기술 구현력이 브랜딩 가치와 사용자 경험(UX) 향상에 기여하는 성과를 만들었습니다.",
  ],
  projects: [
    {
      ...
      techStack: ["AngularJS", "SCSS", "Zeplin", "Git", "Wiki"],
    },
    {
    ...
    },
    {
      ...
      techStack: ["AngularJS", "SCSS", "JavaScript", "Zeplin", "Git", "Wiki"],
      images: [
        {
...
const DOUZONE_EXPERIENCE: Experience = {
  id: "douzone",
  company: "더존 비즈온",
  role: "UI 개발팀 (전임)",
...
const HIVELAB_EXPERIENCE: Experience = {
  id: "hivelab",
  company: "하이브랩",
  role: "UIT 웹표준개발팀 (전임)",
