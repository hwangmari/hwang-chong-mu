-- 가계부 워크스페이스에 "월별 예산" 추가 (monthly_budgets jsonb: { "2026-07": 1800000, ... })
-- 기존 단일 monthly_budget 컬럼은 기본값(레거시)으로 그대로 유지한다.
-- 특정 달에 값이 없으면 클라이언트에서 "직전 달 예산 이어받기"로 해석한다.
--
-- 적용: 이 파일 전체를 Supabase SQL Editor에 붙여넣고 실행.
-- 1) 컬럼 추가  2) get_store가 monthlyBudgets를 반환하도록 교체  3) upsert에 p_monthly_budgets 추가

-- 1) 컬럼 추가
alter table account_book_workspaces
  add column if not exists monthly_budgets jsonb not null default '{}'::jsonb;

-- 2) get_store 교체: workspaces 반환에 'monthlyBudgets' 키 추가 (기존 키 전부 유지)
...
-- 3) upsert_workspace 교체: p_monthly_budgets 파라미터 추가
--    시그니처가 바뀌므로 기존 9-파라미터 버전(p_monthly_budget 포함)을 먼저 제거하고,
--    p_monthly_budgets에 default를 두어 구버전 클라이언트(9인자 호출)도 계속 동작하게 한다.
drop function if exists public.account_book_upsert_workspace(text, text, text, text, integer, jsonb, text, text[], integer);

CREATE OR REPLACE FUNCTION public.account_book_upsert_workspace(
  p_id text,
  p_name text,
  p_type text,
  p_password text,
  p_annual_saving_goal integer,
  p_asset_goal_map jsonb,
  p_owner_user_id text,
  p_member_ids text[],
  p_monthly_budget integer DEFAULT 0,
...
begin
  ...
  values (
    ...
    coalesce(p_monthly_budgets, '{}'::jsonb),
  ...
  set
    ...
    monthly_budgets = excluded.monthly_budgets,
